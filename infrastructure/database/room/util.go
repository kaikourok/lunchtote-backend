...
func (db *RoomRepository) attachPermissions(roomId int, tx *sqlx.Tx) error {
	...
	type MemberPermissions struct {
		...
		CreateChildrenRoom bool   `db:"create_children_room"`
		Type               string `db:"type"`
	}
	...
	for rows.Next() {
		...
		for _, roleId := range roles {
			...
			permissions.CreateChildrenRoom = applyGreaterRolePermission(permissions.CreateChildrenRoom, role.createChildrenRoom)
